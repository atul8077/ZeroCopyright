...
export const Pricing: React.FC = () => {
  ...
  const isCreatorPro = profile?.subscription_tier === 'creator pro' && profile?.subscription_status === 'active';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem', alignItems: 'stretch' }}>
        ...
        <div className="glass-card pricing-card" style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto' }}>
            <button className="btn btn-secondary" onClick={() => handlePlanSelection('Free')} style={{ width: '100%', padding: '0.875rem', fontSize: '1rem', borderRadius: '12px', fontWeight: 600 }}>Get Started</button>
          </div>
        </div>
        ...
        <div className="glass-card pricing-card pro-card" style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column', border: '2px solid var(--accent-color)', position: 'relative', transform: 'scale(1.03)', zIndex: 10, boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ marginTop: 'auto' }}>
            {isPro ? (
              ...
              <button className="btn btn-primary" onClick={() => handlePlanSelection('Pro')} style={{ width: '100%', padding: '0.875rem', fontSize: '1rem', borderRadius: '12px', fontWeight: 600 }}>Choose Pro</button>
            )}
          </div>
        </div>
        ...
        <div className="glass-card pricing-card" style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: 'auto' }}>
            {isCreatorPro ? (
              <button className="btn btn-secondary" onClick={() => navigate('/studio')} style={{ width: '100%', padding: '0.875rem', fontSize: '1rem', borderRadius: '12px', fontWeight: 600, background: 'var(--success-color)', color: '#fff' }}>Open Plan</button>
            ) : (
              <button className="btn btn-secondary" onClick={() => handlePlanSelection('Creator Pro')} style={{ width: '100%', padding: '0.875rem', fontSize: '1rem', borderRadius: '12px', fontWeight: 600 }}>Choose Creator Pro</button>
            )}
          </div>
        </div>
      </div>
